...
class BiLSTM_CRF(nn.Module):
    ...
    def _viterbi_decode(self, feats):
        ...
        for feat_index in range(feats.shape[0]):
            ...
            next_tag_var = gamar_r_l + self.transitions#[5,5]

            viterbivars_t,bptrs_t = torch.max(next_tag_var,dim=1)#[5]

            t_r1_k = torch.unsqueeze(feats[feat_index], 0)#[1,5]
            # print(t_r1_k.shape)
            forward_var_new = torch.unsqueeze(viterbivars_t,0) + t_r1_k#[1,5]
