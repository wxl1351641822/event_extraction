...
class SequenceLabelTask(BaseTask):
    ...
    def _build_net(self):
        ...
        if self.add_crf:
            ...
            for i in range(self.max_events_len):
                self.emission.append(fluid.layers.fc(
                ...
                size = self.emission[-1].shape[1]

                fluid.layers.create_parameter(
                    shape=[size + 2, size], dtype=self.emission[-1].dtype, name='crfw')#转移概率
                self.ret_infers.append(fluid.layers.crf_decoding(
                    input=self.emission[-1], param_attr=fluid.ParamAttr(name='crfw')))
            self.emission=fluid.layers.concat(self.emission,axis=0)
            self.ret_infers=fluid.layers.concat(self.ret_infers,axis=0)
    ...
    def _add_loss(self):
        if self.add_crf:
            labels = fluid.layers.sequence_unpad(self.labels[0],
                                                 self.seq_len_used*self.max_events_len)
    ...
    def _postprocessing(self, run_states):
        ...
        for batch_states in run_states:
            ...
            for length in seq_lens:
                seq_infers = batch_infers[current_id:current_id + length*self.max_events_len]
                seq_result = list(map(id2label.get, seq_infers[1:-1]))
                current_id += length*self.max_events_len if self.add_crf else self.max_seq_len
